...
| **W3** | Sequenziell: ~~`1.B.3-key-rotation`~~ ✅ (#189) → ~~**Adapter-Audit (atomar 4 Inbox-Typen)**~~ ✅ (#190) → `1.B.2-verification` (Rest, fast leer — s.u.) → `1.B.3-sync-recovery + canonical-removal-cleanup` → `1.B.3-discovery-recovery + discovery-attestations` → `1.B.3-device-keys` → `1.B.3-admin-management` → `1.D Demo-Hooks` → `1.E Test-Migration` → `1.C Standalone-Publikation (+ NodeNext-Fix)` | Sync 003 §Capability-JWS + §Envelope; Sync 004 §Recovery; Sync 005 §Admin; Trust 002; Identity 004 |
...
| `1.B.2-verification` (W3, **Rest nach #190**) | — | **Durch #190 fast vollständig erledigt:** alle 5 Demo-Stellen (AttestationService 2, useVerification 3) senden via `deliverInboxMessage`, Empfang via `InboxReceptionHost`/`attestationListener.ts`, VC-JWS reist als `{vcJws}` in spec-konformem `inbox/1.0`. **Rest:** die `DeliveryReceipt.status: 'accepted'`-Spec-Lese (s.u.) + Abschluss-Audit, ob Trust-002-Conformance-Checkliste komplett grün ist. Kandidat: in den sync-recovery-Slice falten statt eigener PR. | Sync 003 §Envelope; Trust 002 |
