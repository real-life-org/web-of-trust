...
| **W3** | Sequenziell: ~~`1.B.3-key-rotation`~~ ✅ (#189) → ~~**Adapter-Audit (atomar 4 Inbox-Typen)**~~ ✅ (#190) → ~~`1.B.2-verification`~~ ✅ (#191, Audit ohne Code) → ~~`1.B.3-sync-recovery + canonical-removal-cleanup`~~ ✅ (#191) → `1.B.3-discovery-recovery + discovery-attestations` → `1.B.3-device-keys` → `1.B.3-admin-management` → `1.D Demo-Hooks` → `1.E Test-Migration` → `1.C Standalone-Publikation (+ NodeNext-Fix)` | Sync 003 §Capability-JWS + §Envelope; Sync 004 §Recovery; Sync 005 §Admin; Trust 002; Identity 004 |
