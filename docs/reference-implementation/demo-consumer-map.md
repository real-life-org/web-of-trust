...
|---|---|---|---|
| `apps/demo/src/runtime/appRuntime.ts` | `@web_of_trust/core/adapters`, `@web_of_trust/core/protocol-adapters` | Concrete adapters belong here now. (Consolidated into one `src/adapters/` bucket; the `protocol-adapters` subpath remains as a backward-compat alias pointing to `adapters/protocol-crypto/`.) | Composition root with explicit adapter packages/entry points. |
...
| `apps/demo/src/context/AdapterContext.tsx` | `InMemoryKeyManagementAdapter` from `@web_of_trust/core/adapters`, passed as the CRDT adapters' `keyManagement` DI param | Composition root wires a concrete `KeyManagementPort` adapter into the CRDT adapters. Acceptable at the composition root; the group-key workflow that orchestrates the port lives in `application/sync/`. | Keep at composition root; durable key persistence is a follow-up adapter sub-slice. |
