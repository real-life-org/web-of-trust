...
---|---|---|---|---|---
Identity seed vault | `IdentitySeedVault`, `SeedStorageAdapter`, `SeedStorageIdentityVault` | Protect, store, unlock, delete, and optionally session-cache identity seed material or opaque seed handles. | Reference `IdentitySeedVault` now avoids raw-seed-returning unlock APIs, the public `@web_of_trust/core/ports` barrel no longer re-exports `SeedStorageAdapter`, and `@web_of_trust/core`/`@web_of_trust/core/application` no longer export `WotIdentity`. The remaining lower-level `SeedStorageAdapter` port type and `SeedStorage`/`WotIdentity` implementations live only behind direct internal imports and still need legacy file deletion before a clean package-level claim. | `narrow/reshape` | Follow-up: delete legacy raw-seed source files and direct-internal-source tests after remaining callers migrate to `IdentityWorkflow`/`PublicIdentitySession`, plus runtime-specific non-extractable-handle posture.
