...
---|---|---|---|---|---
Identity seed vault | `IdentitySeedVault`, `SeedStorageAdapter`, `IndexedDbIdentitySeedVault` | Protect, store, unlock, delete, and optionally session-cache identity seed material or opaque seed handles. | Reference `IdentitySeedVault` now avoids raw-seed-returning unlock APIs, the public `@web_of_trust/core/ports` barrel no longer re-exports `SeedStorageAdapter`, and `@web_of_trust/core`/`@web_of_trust/core/application` no longer export `WotIdentity`. `IndexedDbIdentitySeedVault` is the canonical browser seed-vault adapter; the remaining lower-level `SeedStorageAdapter` port type and `SeedStorage` implementation live only behind that local IndexedDB boundary. | `narrow/reshape` | Follow-up: document runtime-specific non-extractable-handle posture and decide whether the local seed-storage internals should remain public direct-import surfaces.
