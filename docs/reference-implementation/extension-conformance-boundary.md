...
`05-hmc-extensions/H03-gossip.md` | `wot-hmc@0.1` | **Safe-ignore only** | No HMC gossip protocol is implemented. Unknown HMC gossip messages can only be safely ignored unless and until the owning spec defines required dispatch, forwarding, validation, or persistence behavior. `trust-list-delta` authority/forwarding and disclosure regex questions are **blocked pending `wot-spec` clarification** in `wot-spec#43` and `wot-spec#44`. | After `wot-spec#43` and `wot-spec#44`, add schema/vector-backed trust-list-delta parsing and forwarding boundaries. Keep message-dispatch behavior out of scope until the spec defines it.
...
- Trust-list-delta authority, forwarding, and disclosure regex behavior must remain blocked until `wot-spec#43` and `wot-spec#44` resolve the normative boundary.
...
HMC trust-list-delta parsing | `wot-spec#43` and `wot-spec#44` resolved | Schema/vector-backed parser and verifier boundaries only. Forwarding semantics must match the resolved spec.
