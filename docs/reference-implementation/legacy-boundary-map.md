...
`packages/wot-core/src/adapters/protocol-crypto/` | `adapter-only` | Adapter for protocol crypto ports | `WebCryptoProtocolCryptoAdapter` is useful as a browser/runtime implementation of `ProtocolCryptoAdapter`; it must not become protocol authority. (External subpath `@web_of_trust/core/protocol-adapters` bleibt unverändert für Backward-Compat.)
