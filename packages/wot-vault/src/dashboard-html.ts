const SHARED_CSS = `
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .bar { height: 4px; border-radius: 2px; background: #a855f7; display: inline-block; vertical-align: middle; }
`

export function getVaultDashboardHtml(): string {
  return `<!DOCTYPE html>
<html lang="de">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>WoT Vault Dashboard</title>
<style>${SHARED_CSS}</style>
</head>
<body>
<div class="page">
  <div class="header">
    <span class="header-badge bg-purple">Vault</span>
    <div>
      <h1>Encrypted Document Store</h1>
      <div class="header-sub">https://vault.utopia-lab.org</div>
    </div>
  </div>
...
function render(d) {
  const docs = d.topDocs || []
  const maxBytes = Math.max(1, ...docs.map(doc => doc.bytes))

  return \`
    <div class="stats">
      <div class="stat"><div class="stat-val purple">\${d.docCount}</div><div class="stat-label">Dokumente</div></div>
      <div class="stat"><div class="stat-val">\${d.changeCount}</div><div class="stat-label">Changes</div></div>
      <div class="stat"><div class="stat-val">\${d.snapshotCount}</div><div class="stat-label">Snapshots</div></div>
      <div class="stat"><div class="stat-val">\${formatBytes(d.totalBytes)}</div><div class="stat-label">Speicher</div></div>
      <div class="stat"><div class="stat-val">\${d.memoryMB?.toFixed(1) ?? '?'} MB</div><div class="stat-label">RAM</div></div>
    </div>

    <div class="section">
      <div class="section-head">Dokumente nach Gr\\u00f6\\u00dfe (Top 10)</div>
      \${docs.length === 0 ? '<div class="empty">Keine Dokumente</div>' : \`
        <table>
          <tr><th>Doc ID</th><th>Changes</th><th>Gr\\u00f6\\u00dfe</th><th></th><th>Letztes Update</th></tr>
          \${docs.map(doc => \`
            <tr>
              <td class="mono">\${doc.doc_id.slice(0, 16)}...\${doc.doc_id.slice(-6)}</td>
              <td>\${doc.changes}</td>
              <td><span class="badge badge-purple">\${formatBytes(doc.bytes)}</span></td>
              <td><span class="bar" style="width: \${Math.max(4, (doc.bytes / maxBytes) * 80)}px"></span></td>
              <td style="color: #737373">\${formatAge(doc.last_update)}</td>
            </tr>
          \`).join('')}
        </table>
      \`}
    ...
    <div class="section">
      <div class="footer">
        Alle Daten sind Ende-zu-Ende verschl\\u00fcsselt. Der Vault speichert ausschlie\\u00dflich opake, verschl\\u00fcsselte Blobs. Kein Zugriff auf Inhalte m\\u00f6glich.
      </div>
    </div>
  \`
}
