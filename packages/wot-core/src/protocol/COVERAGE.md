...
| `wot-sync@0.1` | focused protocol tests | Sync 003 known-device `device-revoke` disposition | `sync/device-revocation-disposition.ts` | Full for slice | Pure post-signature broker disposition for a caller-supplied exact known `(did, deviceId)` record: active records produce mark-revoked plus exact-device pending-inbox cleanup guidance, already-revoked records are accepted idempotently without metadata overwrite guidance, and unknown-device/error/timestamp validation policy remains deferred to `wot-spec` issues #27, #28, and #32. |
