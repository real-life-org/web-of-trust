/**
 * M1-Tests (Sync 003 Z.466 + Z.620-622): Die Message-ID-History wird erst bei
 * konklusiver Verarbeitung befüllt. Ein nicht-konklusiver Ausgang (unknown
 * space → pending/not-buffered) lässt die id frei: die Relay-Redelivery ist der
 * Recovery-Pfad und wird normal angewendet. Erst danach endet eine weitere
 * Redelivery als Replay mit duplicate-known-ack (Queue-Räumung).
 */
...
async function setup() {
  ...
  const alice = (await createTestIdentity('redelivery-alice')).identity
  const admin = (await createTestIdentity('redelivery-admin')).identity
...
describe('YjsReplicationAdapter — Message-ID-History erst bei konklusiver Verarbeitung (M1)', () => {
